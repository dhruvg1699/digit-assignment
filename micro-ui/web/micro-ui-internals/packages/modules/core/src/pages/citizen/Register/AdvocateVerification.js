import { Button, CardHeader, CardLabel, CardText, Dropdown, Header, LabelFieldPair, TextInput } from "@egovernments/digit-ui-components";
import React, { useState } from "react";
import { states } from "./constants";
import { CardSubHeader, FormComposer } from "@egovernments/digit-ui-react-components";

export const AdvocateVerification = ({ config, advocateRegistration }) => {
  return (
    <FormComposer
      onSubmit={advocateRegistration}
      noBoxShadow
      inline
      submitInForm
      config={config}
      cardStyle={{ maxWidth: "35rem", margin: "auto" }}
      label={"Continue"}
      props={{ text: "HI" }}
    >
      <CardHeader>Advocate Verification</CardHeader>
      <CardText>To ensure the authenticity of your profile, please provide the following details for us to verify</CardText>
    </FormComposer>
  );
};
